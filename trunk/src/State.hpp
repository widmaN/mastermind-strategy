 ...
 * 1. Not all subsets of @c S can be generated by a filter.
 For example, the set containing all but one element of the universe
 is not reachable (this can be proved by exhaustively testing each
 codeword as the first guess; in all cases, the max partition is
 less than 1440 elements. Hence any subset with more than 1440 
 elements is unreachable
 *
 * There are so many subsets (2^5040-1) non-empty subsets.
 * However, there are only a fraction of reachable subsets.
 If we consider evuialence, the number is smaller and tractable.
 *
 * In the future, we probably don't need to generate the universe;
 * instead, we can only generate the canonical elements.
 *
 ...
 * of guesses.
 *
 * Note that filter equivalence is strictly weaker than partition
 * equivalence. For example, consider
 *  1234:0A0B 
 * Then for the next guess,
 *  1156 or 1256 are obviously the same (they both contain excluded
 * colors 1, 2). However, 1156 and 1256 are clearly not isomorphs.
 * This is because filter equivalence doesn't use the info of the
 * feedback.
 *
 * One way to deal with this is to use both. If two guesses are
 * equivlant under any measure, then they are equivalent.
 *
 * Or, we could combine the two. When there are a lot of possibilities,
 * we compute filter equivalence. When there are a few possibilities,
 * we compute partition equivalence.

*
* Trivial strategy is the same as max_parts and min_max.
 * It is also same as minavg? But it's not the same as simple.
 *
 *
 *
 * 
