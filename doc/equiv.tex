...
\begin{definition}
(Composite equivalence) Let $R_1$ and $R_2$ be two equivalence relations defined on the codeword set $\vS_0$. Then $R$ is a \emph{composite equivalence relation} of $R_1$ and $R_2$ if for any $g_1, g_2 \in \vS_0$, $g_1 R g_2$ if and only if $g_1 R_1 g_2$ or $g_1 R_2 g_2$. We denote it as $R = R_1 \vee R_2$.
\end{definition}

[Colarary] If $R_1$ is \emph{finer} than $R_2$, then $R = R_1$.

Following this, we can define a compound equivalence filter $\mathcal{F}$ given two filters $\mathcal{F}_1$ and $\mathcal{F}_2$ by filtering the candidates using the compound equivalence relation.

To actually implement a composite equivalence filter, one possibility is to implement it using \emph{disjoint sets}. That is, we separately partition the candidate set $G$ by $E_1$ and $E_2$, and then merge the equivalence classes.

However, this approach is only useful when the two equivalence relations are not finer or coarser; if one equivalence relation is finer than the other, then there is no need to do this anyway. It doesn't reduce the computational overhead.

So, we instead use chaining. That is, given two filters, we first filter to get $G_1 = \text{filter}(G; \mathcal{F}_1)$, then we do $G_2 = \text{filter}(G_1; \mathcal{F}_2)$.

This works under some important conditions. Let's give a counter-example. Suppose there are four elements, $1, 2, 3, 4$, and two equivalence relations $R_1$ and $R_2$, such that
\[
R_1 = \{ \{ 1, 2 \}, \{ 3, 4 \} \} ,
\]
and
...
R_2 = \{ \{ 1, 3 \}, \{ 2, 4 \} \} .
\]
A composite equivalence $R$ can be defined. Obviously we have
\[
R = \{ \{ 1, 2, 3, 4 \} \} .
\]
Suppose filter 1 returns $\{ [2], [3] \}$, and this is fed to filter 2. Which will natually return $\{ [2], [3] \}$. However, it should have returned only one element.

This counter-example means we must put some requirement on how a filter returns representative elements. We have the following requirement:
...
\begin{proof}
Before going into details, we illustrate the idea using an example of 7 codewords. A $\times$ sign indicates a representative of one of the canonical filters, and a $\otimes$ sign indicates a representative of both filters.
\[
